...
export function ProjectFinancial({ projectId }: { projectId: string }) {
  ...
  const snapshot = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.rpc("project_profit_snapshot", {
  ...
  const deposits = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("deposits").select("*").eq("project_id", projectId).order("created_at"))
        .data ?? [],
  });
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Kpi
          ...
          value={formatMoney(snap?.approved_revenue ?? 0)}
          tone="navy"
        />
        <Kpi
          icon={Receipt}
          label="Invoiced"
          value={formatMoney(snap?.invoiced_revenue ?? 0)}
          tone="sky"
        />
        <Kpi
          icon={Wallet}
          label="Paid"
          value={formatMoney(snap?.paid_revenue ?? 0)}
          tone="emerald"
        />
        <Kpi
          icon={AlertCircle}
        ...
        <CardContent>
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-md bg-muted/40 px-3 py-2 text-xs">
            <span className="text-muted-foreground">
              Variance (approved − estimate, cost adjusted)
            </span>
            <span
              className={`tabular-nums font-semibold ${(snap?.variance ?? 0) < 0 ? "text-destructive" : "text-emerald-700"}`}
            >
      ...
      <DepositsSection
        projectId={projectId}
        proposalId={acceptedProposal.data?.id ?? null}
      ...
      <InvoicesSection
        projectId={projectId}
        proposal={acceptedProposal.data}
        invoices={invoices.data ?? []}
        onChanged={invalidateAll}
      />

      {/* PROGRESS BILLINGS */}
      <ProgressBillingsSection
        projectId={projectId}
        items={progress.data ?? []}
        onChanged={invalidateAll}
      />
              ...
              return (
                <div key={d.id} className="flex flex-wrap items-center justify-between gap-2 py-2">
                  <div className="flex items-center gap-2">
                    <span
                      className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${meta.color}`}
                    >
                      {meta.label}
                    </span>
...
function InvoicesSection({
  ...
  const createBlank = useMutation({
    ...
    onSuccess: () => {
      toast.success("Blank invoice created.");
      onChanged();
    },
    onError: (e: any) => toast.error(e.message),
  });
      ...
      <CardContent>
        {invoices.length === 0 ? (
          <div className="rounded-md border border-dashed py-8 text-center text-xs text-muted-foreground">
            No invoices yet.{" "}
            {proposal
              ? "Generate one from the approved proposal above."
              : "Approve a proposal to generate an invoice automatically."}
          </div>
        ) : (
        ...
        <div className="flex items-center gap-3">
          <div className="text-right text-xs">
            ...
            <div className="text-muted-foreground">
              Balance:{" "}
              <span className="tabular-nums font-semibold">
                {formatMoney(Number(invoice.balance_due))}
              </span>
            </div>
          </div>
          <Button
            ...
            disabled={invoice.status === "void" || Number(invoice.balance_due) <= 0}
          >
          ...
          {invoice.status !== "void" && (
            <Button size="sm" variant="ghost" onClick={() => voidInv.mutate()}>
...
function InvoiceLineItems({ invoice, onChanged }: { invoice: any; onChanged: () => void }) {
  ...
  const recompute = async (invoiceId: string) => {
    const { data } = await supabase
      ...
      .select("line_total,tax_rate,quantity,unit_price")
    ...
    const tax = (data ?? []).reduce(
      (s, r) => s + Number(r.quantity) * Number(r.unit_price) * Number(r.tax_rate ?? 0),
      0,
    );
    await supabase
      .from("invoices")
                ...
                <tr key={r.id}>
                  <td className="py-1">
                    <Input
                      ...
                      onBlur={(e) =>
                        update.mutate({ id: r.id, patch: { description: e.target.value } })
                      }
                    />
                  </td>
                  <td>
                    <Input
                      ...
                      onBlur={(e) =>
                        update.mutate({ id: r.id, patch: { quantity: Number(e.target.value) } })
                      }
                    />
                  </td>
                  <td>
                    <Input
                      ...
                      onBlur={(e) =>
                        update.mutate({ id: r.id, patch: { unit_price: Number(e.target.value) } })
                      }
                    />
                  </td>
                  <td className="text-right tabular-nums font-semibold">
                    {formatMoney(Number(r.line_total))}
                  </td>
                  <td>
...
function PaymentRow({ payment, onChanged }: { payment: any; onChanged: () => void }) {
  ...
  return (
    <div
      className={`flex items-center justify-between py-0.5 ${payment.is_void ? "opacity-50 line-through" : ""}`}
    >
      <span>
      ...
      <span className="flex items-center gap-2">
        ...
        {!payment.is_void && (
          <Button size="sm" variant="ghost" onClick={() => voidPay.mutate()}>
            <Ban className="h-3 w-3" />
          </Button>
        )}
      </span>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button onClick={() => record.mutate()} disabled={record.isPending || !Number(amount)}>
            Record
          </Button>
        </DialogFooter>
      </DialogContent>
...
function ProgressBillingsSection({
  projectId,
  items,
  onChanged,
}: {
  ...
  const remove = useMutation({
    mutationFn: async (id: string) => {
      await supabase.from("progress_billings").delete().eq("id", id);
    },
    onSuccess: onChanged,
  });
  ...
  return (
    <Card>
      ...
      <CardContent>
        {items.length === 0 ? (
          <div className="rounded-md border border-dashed py-6 text-center text-xs text-muted-foreground">
            Use progress billing for phased projects — track percent complete and retainage per
            draw.
          </div>
        ) : (
          <div className="overflow-x-auto">
                ...
                {items.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="text-right">
                      <Input
                        type="number"
                        defaultValue={r.percent_complete}
                        className="ml-auto h-7 w-20 text-right text-xs"
                        onBlur={(e) =>
                          update.mutate({
                            id: r.id,
                            patch: { percent_complete: Number(e.target.value) },
                          })
                        }
                      />
                    </td>
                    <td className="text-right">
                      <Input
                        type="number"
                        defaultValue={r.amount_due}
                        className="ml-auto h-7 w-28 text-right text-xs"
                        onBlur={(e) =>
                          update.mutate({ id: r.id, patch: { amount_due: Number(e.target.value) } })
                        }
                      />
                    </td>
                    <td className="text-right">
                      <Input
                        type="number"
                        defaultValue={r.retainage}
                        className="ml-auto h-7 w-24 text-right text-xs"
                        onBlur={(e) =>
                          update.mutate({ id: r.id, patch: { retainage: Number(e.target.value) } })
                        }
                      />
                    </td>
                    <td className="text-right">
                      <Select
                        value={r.status}
                        onValueChange={(v) => update.mutate({ id: r.id, patch: { status: v } })}
                      >
  ...
  return (
    <Card>
      <CardContent className="p-4">
        ...
        <div className={`font-display text-2xl font-bold tabular-nums ${tones[tone] ?? ""}`}>
          {value}
        </div>
      </CardContent>
    </Card>
...
function ProfitTile({
  ...
  return (
    <div className={`rounded-md border p-3 ${highlight ? "border-gold/40 bg-gold/5" : ""}`}>
      ...
      <div
        className={`mt-1 font-display text-lg font-bold tabular-nums ${muted ? "text-slate-700" : positive ? "text-emerald-700" : "text-destructive"}`}
      >
        {formatMoney(value)}
      </div>
    </div>
...
function SendInvoiceButton({
  ...
  return (
    <Button
      ...
      disabled={busy}
      onClick={send}
      title={sentAt ? `Resend invoice (last sent ${formatDate(sentAt)})` : "Send invoice by email"}
    >
...
function InvoicePortalLinkButton({
  invoiceId,
  onChanged,
}: {
  invoiceId: string;
