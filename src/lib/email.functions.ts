...
export const sendInvoiceEmailFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Record sent_at only after successful email delivery
    await supabase
      .from("invoices")
...
export const sendPortalInvitationEmailFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      throw new Error(
        error?.message
          ? `${error.message}. PIN was rotated; retry send to deliver the current PIN.`
          : "Email delivery failed. PIN was rotated; retry send to deliver the current PIN.",
      );
    }
