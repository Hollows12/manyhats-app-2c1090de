...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Leads, estimates, proposals, field capture, and job intelligence for ManyHats Construction LLC." },
    ],
  }),
  component: Landing,
});
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button asChild className="bg-gold text-gold-foreground hover:bg-gold/90">
              <Link to="/auth" search={{ invite: undefined }}>Open dashboard <ArrowRight className="ml-1 h-4 w-4" /></Link>
            </Button>
          </div>
        </div>
      </header>
        ...
        <div className="grid items-center gap-14 md:grid-cols-[1.2fr_1fr]">
          <div>
            ...
            <h1 className="mt-6 font-display text-5xl font-bold leading-[1.05] md:text-6xl">
              The contractor command center for{" "}
              <span className="text-gold">{COMPANY.name}.</span>
            </h1>
            <p className="mt-6 max-w-xl text-lg text-ivory/80">
              Leads. Clients. Field capture. Estimates. Proposals. Concepts. Job costing.
              Every project ties back to the same source of truth — built for {COMPANY.specialties.toLowerCase()}.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Button asChild size="lg" className="bg-gold text-gold-foreground hover:bg-gold/90">
                <Link to="/auth" search={{ invite: undefined }}>Get started <ArrowRight className="ml-2 h-4 w-4" /></Link>
              </Button>
              <a
          ...
          <div className="grid gap-4">
            {[
              { icon: HardHat, title: "Field-first", body: "Capture photos, measurements, and notes from a phone. No app required." },
              { icon: Hammer, title: "From lead to closeout", body: "16 modules — leads, estimates, proposals, concepts, job costing, and a private knowledge base." },
              { icon: ShieldCheck, title: "Protective by default", body: "No final pricing without confirmed measurements. Real photos always last in the proposal." },
            ].map((card) => (
              <div key={card.title} className="rounded-lg border border-white/10 bg-white/5 p-5 backdrop-blur">
