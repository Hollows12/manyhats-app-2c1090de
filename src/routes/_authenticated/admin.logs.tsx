  ...
  return (
    <div className="space-y-6 p-4 md:p-8">
      ...
      <p className="text-sm text-muted-foreground">
        Full audit view of activity, errors, and immutable change history. Only admins can access
        this page.
      </p>
      ...
      <Tabs defaultValue="activity">
        <TabsList>
          ...
          <TabsTrigger value="errors">
            <AlertTriangle className="mr-1 h-3 w-3" /> Errors
          </TabsTrigger>
          <TabsTrigger value="audit">
            <ClipboardList className="mr-1 h-3 w-3" /> Audit trail
          </TabsTrigger>
        </TabsList>
...
function ActivityPanel() {
  ...
  const rows = (q.data ?? []).filter(
    (r: any) => !search || JSON.stringify(r).toLowerCase().includes(search.toLowerCase()),
  );
      ...
      <CardContent>
        {q.isLoading ? (
          ...
          <EmptyState
            ...
            description="Staff actions will show up here."
          />
        ) : (
...
function ErrorsPanel() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-sm">Errors ({rows.length})</CardTitle>
      </CardHeader>
      <CardContent>
        {q.isLoading ? (
          <Loading />
          ...
          <EmptyState
            ...
            description="Nice — nothing to see here."
          />
        ) : (
          <div className="divide-y divide-border text-sm">
            {rows.map((r: any) => (
...
function AuditPanel() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-sm">Audit trail ({rows.length})</CardTitle>
      </CardHeader>
      <CardContent>
        {q.isLoading ? (
          <Loading />
          ...
          <EmptyState
            ...
            description="Sensitive changes will be recorded here with before/after diffs."
          />
        ) : (
          <div className="divide-y divide-border text-sm">
            {rows.map((r: any) => (
